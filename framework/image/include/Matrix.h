...
public:
    ...
    */
    float operator[](int index) const {
        Tengine_ASSERT((unsigned)index < 9);
        return fMat[index];
    }
    ...
    */
    void mapPoints(Point dst[], const Point src[], int count) const {
        Tengine_ASSERT((dst && src && count > 0) || 0 == count);
        // no partial overlap
        Tengine_ASSERT(src == dst || &dst[count] <= &src[0] || &src[count] <= &dst[0]);
    ...
    */
    friend Tengine_PUBLIC bool operator==(const Matrix& a, const Matrix& b);
...
private:
    ...
    */
    ...
    void setTypeMask(int mask) {
        ...
        Tengine_ASSERT(kUnknown_Mask == mask || (mask & kAllMasks) == mask ||
    ...
    void orTypeMask(int mask) {
        Tengine_ASSERT((mask & kORableMasks) == mask);
        fTypeMask = (uint8_t)(fTypeMask | mask);
    }

    void clearTypeMask(int mask) {
        // only allow a valid mask
        Tengine_ASSERT((mask & kAllMasks) == mask);
    ...
    */
    ...
    static MapXYProc GetMapXYProc(TypeMask mask) {
        Tengine_ASSERT((mask & ~kAllMasks) == 0);
